...
class CNSNetworkController: NSObject {
    ...
    @objc func UPDATE(pokemon: CNSPokemon) {
        ...
        URLSession.shared.dataTask(with: request) { (data, _, error) in
            ...
            do {
                ...
                _ = NSError(domain: "NetworkControllerErrorDomain", code: 4, userInfo: nil);
                return;
            }
        }.resume()
    }
    ...
    @objc func GET(completion: @escaping CompletionHandler = { _,_  in }) {
           ...
           guard let requestURL = urlComponents?.url else { NSLog("URL is nil"); completion(nil, NSError(domain: "NetworkControllerErrorDomain", code: 0, userInfo: nil)); return }
           var request = URLRequest(url: requestURL)
           request.httpMethod = "GET"
