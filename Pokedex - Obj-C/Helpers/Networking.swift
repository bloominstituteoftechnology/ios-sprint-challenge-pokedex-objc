...
@objc class PokemonAPI: NSObject {
    ...
    @objc func fetchAllPokemon(completion: @escaping ([CLBPokemon]?, Error?) -> Void) {
        URLSession.shared.dataTask(with: URL(string: "https://pokeapi.co/api/v2/pokemon/?limit=1000")!) { (data, _, error) in
            ...
            var results = [CLBPokemon]()
            for item in array {
                results.append(CLBPokemon(dictionary: item))
            }
            completion(results, nil)
        }.resume()
    }
    
    @objc func fillInDetails(for pokemon: CLBPokemon) {
